...
@interface HBannerView ()<UIScrollViewDelegate>
...
- (instancetype)initViewWithFrame:(CGRect)frame autoPlayTime:(NSTimeInterval)playTime imagesArray:(NSArray *)imagesArray textsArray:(NSArray *)textsArray clickCallBack:(void (^)(NSInteger))clickCallBack
{
...
- (void)setImageView:(UIImageView *)imageView atIndex:(NSInteger)index
{
    ...
    if (index < _imagesArray.count) {
        ...
        CGImageRef imageRef = CGImageCreateWithImageInRect(cgRef, CGRectMake(0, 0.5 * (image.size.height - (image.size.width * view_HEIGHT / view_WIDTH)), image.size.width, image.size.width * view_HEIGHT / view_WIDTH));
        UIImage *thumbScale = [UIImage imageWithCGImage:imageRef];
        CGImageRelease(imageRef);
        image = thumbScale;
        
        imageView.image = image;
        NSString *string = (NSString *)_textsArray[index];
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, imageView.bounds.size.height - 50, imageView.bounds.size.width - 30, 30)];
        label.textColor = [AppUIModel UIViewMainColorII];
